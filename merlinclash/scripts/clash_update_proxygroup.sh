...
url_main="https://raw.githubusercontent.com/flyhigherpi/merlinclash_clash_binary/master/proxy-group"
url_back=""
...
get_latest_version(){
	...
	curl --connect-timeout 10 -s $url_main/common_rule/lastest.txt > /tmp/proxygroup_latest_info.txt
	if [ "$?" == "0" ];then
...
get_latest_gameversion(){
	rm -rf /tmp/proxygroup_latest_gameinfo.txt
	flag="1"
	...
	curl --connect-timeout 10 -s $url_main/game_rule/gamelastest.txt > /tmp/proxygroup_latest_gameinfo.txt
	if [ "$?" == "0" ];then
		if [ -z "`cat /tmp/proxygroup_latest_gameinfo.txt`" ];then 
			echo_date "获取游戏规则文件最新版本失败！" >> $LOG_FILE
			failed_warning_clash
		fi
		if [ -n "`cat /tmp/proxygroup_latest_gameinfo.txt|grep "404"`" ];then
			echo_date "error:404 | 获取游戏规则文件最新版本失败！" >> $LOG_FILE
			failed_warning_clash
		fi
		if [ -n "$(cat /tmp/proxygroup_latest_gameinfo.txt|grep "500")" ];then
			echo_date "error:500 | 获取游戏规则文件最新版本失败！" >> $LOG_FILE
			failed_warning_clash
		fi
		ggVERSION=$(cat /tmp/proxygroup_latest_gameinfo.txt | sed 's/g//g') || 0
	
		echo_date "检测到游戏规则文件最新版本：g$ggVERSION" >> $LOG_FILE
		if [ ! -f "/jffs/softcenter/merlinclash/yaml/proxy-group-game.yaml" ];then
			echo_date "规则文件丢失！重新下载！" >> $LOG_FILE
			CUR_VER="0"
		else
...
move_proxygroup(){
	...
	if [ "$flag" == "0" ]; then
		...
		if [ "$?" == "0" ];then
			...
			chmod +x /jffs/softcenter/merlinclash/yaml/proxy-group.yaml
	...
	if [ "$flag" == "1" ]; then
		...
		if [ "$?" == "0" ];then
			echo_date 解压完成！>> $LOG_FILE
			mv /tmp/proxygroup/proxy-group-game.yaml /jffs/softcenter/merlinclash/yaml/proxy-group-game.yaml
			chmod +x /jffs/softcenter/merlinclash/yaml/proxy-group-game.yaml
